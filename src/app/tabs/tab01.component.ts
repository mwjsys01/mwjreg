...
export class Tab01Component implements OnInit {
  ...
  read_Data(value:number):void {
    ...
    .subscribe(({ data }) => {
      ...
      for ( let i=0;i<data.tmpshop_tblstock.length;i=i+1 ){
        adInf = [{ gcode:data.tmpshop_tblstock[i].gcode,gname:data.tmpshop_tblstock[i].gname,stock:+data.tmpshop_tblstock[i].stock,
          price:[0,data.tmpshop_tblstock[i].price1,data.tmpshop_tblstock[i].price2,data.tmpshop_tblstock[i].price3,data.tmpshop_tblstock[i].price4,data.tmpshop_tblstock[i].price5]}];
        adBtn = { categ: data.tmpshop_tblstock[i].categ, ginfo: adInf } ;
  ...
  　let lctype   : String;
    ...
    if (this.hedsrv.chSHOP && this.hedsrv.chCMPE) {

    } else if (this.hedsrv.chSHOP) {
      lctype = 'SHOP';
    } else if (this.hedsrv.chCMPE) {
      lctype = 'CMPE';
    }
